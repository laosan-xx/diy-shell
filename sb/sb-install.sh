...
get_ip() {
    # 优先尝试 IPv4
    export ip=$(curl -s -4 --connect-timeout 5 ip.sb 2>/dev/null)

    # 如果 IPv4 失败，尝试 IPv6（可选）
    if [[ -z "$ip" ]]; then
        export ip=$(curl -s -6 --connect-timeout 5 ip.sb 2>/dev/null)
    fi

    # 如果还是空，可以 fallback 到其他备用源（可选）
    if [[ -z "$ip" ]]; then
        export ip=$(curl -s -4 --connect-timeout 5 ifconfig.me/ip 2>/dev/null)
    fi
}
